...
class Clip():
    ...
    def __init__(self, landmark_fname):
        ...
        self.T_l_all = {'l_eye_all':[]}
        self.T_m_all = {'m_eye_all':[]}
...
#####        
        ...
        for i in range(self.nframes): 
            ...
            T__l = (np.linalg.norm(landmarks_frame[43] - landmarks_frame[47]) + \
                    np.linalg.norm(landmarks_frame[44] - landmarks_frame[46]))\
                    / 2 / np.linalg.norm(landmarks_frame[42] - landmarks_frame[45])  
            ...
            if T__m >= 0.229:
                #print(temp)
                A_open.append(self.eyesizes['momentary_mean'])
                #eyesize_open =  temp[self.eyesizes['momentary_mean'] > T]
                #A_open.append(temp)
                u = u + 1
                #print(u)
        ...
        ratio = s / 300.0  
        print(self.video_fname, s, u,np.mean(A_open), ratio)
